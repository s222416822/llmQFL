...
class Device:
    ...
    def fine_tune_llm(self):
        ...
        print("Tokenizing dataset for LLM fine tunning")
        ...
        if self.peft_method == "lora":
            ...
            quant_config = BitsAndBytesConfig(
                ...
                llm_int8_enable_fp32_cpu_offload=True  # Offload some modules to CPU in FP32
            )
        ...
        if self.peft_method != "None":
            ...
            lora_config = LoraConfig(
                r=8, # Rank for low-rank updates
                lora_alpha=8,
                # lora_dropout=0.1,  # Dropout to prevent overfitting
                task_type=TaskType.SEQ_CLS,
                # target_modules=["attn"],  # Target GPT-2 attention modules for LoRA
                # target_modules=["attn.c_attn", "attn.c_proj"],  # Modify target names if necessary
                # bias="none"'
                fan_in_fan_out=True
            )
        ...
        training_args = TrainingArguments(
            output_dir=f"{self.log_folder}/llm_folder/trainer/device_{self.device_id}_trainer",
            evaluation_strategy="epoch",
            # "epoch": Evaluation is done at the end of each epoch.
            # save_strategy="epoch", #"epoch": Save is done at the end of each epoch.
            save_strategy="epoch",
            num_train_epochs=2,
    ...
    def build_qcnn(self):
        ...
        ansatz.compose(conv_layer(self.num_qubits, "c1_{}".format(self.device_id)), list(range(self.num_qubits)), inplace=True)
        ansatz.compose(pool_layer([0, 1], [2, 3], "p1_{}".format(self.device_id)), list(range(self.num_qubits)), inplace=True)
        ansatz.compose(conv_layer(self.num_qubits, "c2_{}".format(self.device_id)), list(range(self.num_qubits)), inplace=True)
        ...
        observables = [SparsePauliOp.from_list([("Z" + "I" * (self.num_qubits - 1), 1)]) for _ in range(self.num_classes)]
        qnn = EstimatorQNN(
