...
def main():
    ...
    if USE_SUBSET:
        ...
        dataset['validation'] = dataset['validation'].select(
            range(min(VALIDATION_SUBSET_SIZE, len(dataset['validation']))))

    server_validation_dataset = dataset["validation"]
    server_test_dataset = dataset["test"]
    full_train_dataset = dataset["train"]

    # Distribute training data among devices, and perform train-test split within each device
    ...
    # Initialize the server device with test set for QCNN (no training for server)
    ...
    # server_device = Device(num_devices, maxiter, server_validation_dataset, server_test_dataset, num_qubits, num_classes, log_folder, lora, use_llm, is_server=True)
