...
import torch
from config import TRAIN_SIZE, TEST_SIZE, NUM_DEVICES
...
def load_and_process_data():
    ...
    model_cls.resize_token_embeddings(len(tokenizer)) # Resize embeddings for the new pad_token
    ...
    # Split into train and test DatasetDict for LLaMA
    dds = DatasetDict({
    ...
    encoded_sequences_np_reshaped = features_encoded_sequences_3D_np.reshape(features_encoded_sequences_3D_np.shape[0],
                                                                         -1)
    ...
    # )
    alldevices_train_features, server_test_features, alldevices_train_labels, server_test_labels = train_test_split(
        features_encoded_pca, labels_encoded_sequences_3D_np, train_size=0.8, random_state=42
    )
